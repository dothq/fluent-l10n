...
class L10n {
    constructor(args) {
        ...
        if (!args.localesDirectory)
            throw new Error(`Unable to find any locales. Set the localesDirectory parameter.`);
        if (!args.filter)
                ...
                if (args.loader) {
                    ...
                    try {
                        for (var _b = __asyncValues(args.availableLocales), _c; _c = yield _b.next(), !_c.done;) {
                            ...
                            const path = (0, path_1.resolve)(args.localesDirectory, `${locale}.ftl`);
                            const ftl = (0, fs_1.readFileSync)(path, { encoding: "utf-8" });
                            data[locale] = ftl;
                        }
                    }
